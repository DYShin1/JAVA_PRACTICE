...
public class Application {
        ...
        while (true) {
            ...
            switch (input){
                case 1: mm.selectAllMembers();
                    break;
                case 2: mm.selectMembers(chooseMemberNo());
                    break;
    ...
    /* 설명. 사용자로부터 회원번호를 제외한 정보를 입력 받아(회원가입용 정보) Memeber타입으로 반환하는 메소드(파싱 및 가공처리) */
    private static Member signUp() {
        ...
        /* 필기.
         *  회원으로부터 회원가입을 위한 정보를 입력받아 Member타입객체 하나로 가공 처리할 때 방법이 두 가지가 있다.
         *  1. 생성자 방식(장: 한줄 코딩 가능, 단: 따로 생성자 추기)
