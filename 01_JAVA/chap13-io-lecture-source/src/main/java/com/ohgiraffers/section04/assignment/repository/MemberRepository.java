...
//        }
    ...
    private int saveMember(Member member) {
        ...
        try {
            moo = new MyObjectOutput(new BufferedOutputStream(new FileOutputStream("src/main/java/com/ohgiraffers/section04/assignment/db/memberDB.dat")));
    ...
    public int deleteMember(int memNo) {
        for (int i = 0; i < memberList.size(); i++) {
            if(memberList.get(i).getMemNo() == memNo){              // 지울려는 회원과 같은 회원 번호인 index 찾기
                ...
                /* 설명. 프로그램 상에서 회원을 관리하는 memberList 뿐 아니라 DB(회원 파일)도 같이 작용되게 함. */
                memberList.remove(i);
