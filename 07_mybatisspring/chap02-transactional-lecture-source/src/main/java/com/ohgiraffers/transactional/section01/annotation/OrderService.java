...
public class OrderService {
    ...
    * */
    ...
    /* 설명. 서비스 메소드 내부에서 모든 DML관련 작업에 예외 없이 잘 동작하면 commit, 예외가 발생하면 rollback이 작용된다. */
    /* 설명.
     *  전파행위 옵션
...
//                orderInfo.getOrderMenus().stream().map(
//                        dto -> {
...
//        );
        ...
        for (OrderMenuDTO orderMenuDTO : orderMenuDTOS) {
            oMenus.add(new OrderMenu(orderMenuDTO.getMenuCode(), orderMenuDTO.getOrderAmount()));
        }
    ...
    /* 설명. 주문건에 대한 총 합
        계 금액 계산 메소드(orderMenus: 사용자의 주문내용, menus: 조회된 메뉴 전체 내용) */
